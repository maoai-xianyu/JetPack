 ...
 * zhangkun
 * <p>
 * 源码：https://github.com/lihangleo2/ShadowLayout
 */
...
public class ShadowLayoutMY extends FrameLayout {
    ...
    public ShadowLayoutMY(Context context, AttributeSet attrs) {
        this(context, attrs, 0);
    }


    public ShadowLayoutMY(Context context, AttributeSet attrs, int defStyleAttr) {
    ...
    public void setShadowLimit(int mShadowLimit) {
        if (isShowShadow) {
            int dip5 = (int) getContext().getResources().getDimension(R.dimen.maoyan_common_shadow_layout_dp_5);
    ...
    private void initAttributes(AttributeSet attrs) {
        TypedArray attr = getContext().obtainStyledAttributes(attrs, R.styleable.ShadowLayoutMY);
        if (attr == null) {
            return;
        }
        ...
        try {
            //默认是显示
            isShowShadow = !attr.getBoolean(R.styleable.ShadowLayoutMY_my_shadowHidden, false);
            leftShow = !attr.getBoolean(R.styleable.ShadowLayoutMY_my_shadowHiddenLeft, false);
            rightShow = !attr.getBoolean(R.styleable.ShadowLayoutMY_my_shadowHiddenRight, false);
            bottomShow = !attr.getBoolean(R.styleable.ShadowLayoutMY_my_shadowHiddenBottom, false);
            topShow = !attr.getBoolean(R.styleable.ShadowLayoutMY_my_shadowHiddenTop, false);
            mCornerRadius = attr.getDimension(R.styleable.ShadowLayoutMY_my_cornerRadius, getResources().getDimension(R.dimen.maoyan_common_shadow_layout_dp_0));
            mCornerRadius_leftTop = attr.getDimension(R.styleable.ShadowLayoutMY_my_cornerRadius_leftTop, -1);
            mCornerRadius_leftBottom = attr.getDimension(R.styleable.ShadowLayoutMY_my_cornerRadius_leftBottom, -1);
            mCornerRadius_rightTop = attr.getDimension(R.styleable.ShadowLayoutMY_my_cornerRadius_rightTop, -1);
            mCornerRadius_rightBottom = attr.getDimension(R.styleable.ShadowLayoutMY_my_cornerRadius_rightBottom, -1);

            //默认扩散区域宽度
            mShadowLimit = attr.getDimension(R.styleable.ShadowLayoutMY_my_shadowLimit, 0);
            if (mShadowLimit == 0) {
            ...
            //x轴偏移量
            mDx = attr.getDimension(R.styleable.ShadowLayoutMY_my_shadowOffsetX, 0);
            //y轴偏移量
            mDy = attr.getDimension(R.styleable.ShadowLayoutMY_my_shadowOffsetY, 0);
            mShadowColor = attr.getColor(R.styleable.ShadowLayoutMY_my_shadowColor, getResources().getColor(R.color.maoyan_common_default_shadow_color));

            selectorType = attr.getInt(R.styleable.ShadowLayoutMY_my_shapeMode, 1);
            isSym = attr.getBoolean(R.styleable.ShadowLayoutMY_my_shadowSymmetry, true);

            //背景颜色的点击(默认颜色为白色)
            mBackGroundColor = getResources().getColor(R.color.maoyan_common_default_shadowback_color);

            Drawable background = attr.getDrawable(R.styleable.ShadowLayoutMY_my_layoutBackground);
            if (background != null) {
            ...
            Drawable trueBackground = attr.getDrawable(R.styleable.ShadowLayoutMY_my_layoutBackground_true);
            ...
            if (mBackGroundColor_true != -101 && layoutBackground != null) {
                throw new UnsupportedOperationException("使用了ShadowLayoutMY_my_layoutBackground_true属性，必须先设置ShadowLayoutMY_my_layoutBackground属性。且设置颜色时，必须保持都为颜色");
            }

            if (layoutBackground == null && layoutBackground_true != null) {
                throw new UnsupportedOperationException("使用了ShadowLayoutMY_my_layoutBackground_true属性，必须先设置ShadowLayoutMY_my_layoutBackground属性。且设置图片时，必须保持都为图片");
            }

            //边框颜色的点击
            stroke_color = attr.getColor(R.styleable.ShadowLayoutMY_my_strokeColor, -101);
            stroke_color_true = attr.getColor(R.styleable.ShadowLayoutMY_my_strokeColor_true, -101);

            if (stroke_color == -101 && stroke_color_true != -101) {
                throw new UnsupportedOperationException("使用了ShadowLayoutMY_my_strokeColor_true属性，必须先设置ShadowLayoutMY_my_strokeColor属性");
            }

            stroke_with = attr.getDimension(R.styleable.ShadowLayoutMY_my_strokeWith, dip2px(1));
            //规定边框长度最大不错过7dp
            if (stroke_with > dip2px(7)) {
                stroke_with = dip2px(5);
            }
            ...
            Drawable clickAbleFalseBackground = attr.getDrawable(R.styleable.ShadowLayoutMY_my_layoutBackground_clickFalse);
            ...
            startColor = attr.getColor(R.styleable.ShadowLayoutMY_my_startColor, -101);
            centerColor = attr.getColor(R.styleable.ShadowLayoutMY_my_centerColor, -101);
            endColor = attr.getColor(R.styleable.ShadowLayoutMY_my_endColor, -101);
            if (startColor != -101) {
                //说明设置了渐变色的起始色
                if (endColor == -101) {
                    throw new UnsupportedOperationException("使用了ShadowLayoutMY_my_startColor渐变起始色，必须搭配终止色ShadowLayoutMY_my_endColor");
                }
            }


            angle = attr.getInt(R.styleable.ShadowLayoutMY_my_angle, 0);
            if (angle % 45 != 0) {
            ...
            if (selectorType == 3) {
                ...
                if (mBackGroundColor == -101 || mBackGroundColor_true == -101) {
                    throw new NullPointerException("使用了ShadowLayout的水波纹，必须设置使用了ShadowLayoutMY_my_layoutBackground和使用了ShadowLayoutMY_my_layoutBackground_true属性，且为颜色值");
                }
            ...
            mTextViewResId = attr.getResourceId(R.styleable.ShadowLayoutMY_my_bindTextView, -1);
            textColor = attr.getColor(R.styleable.ShadowLayoutMY_my_textColor, -101);
            textColor_true = attr.getColor(R.styleable.ShadowLayoutMY_my_textColor_true, -101);
            text = attr.getString(R.styleable.ShadowLayoutMY_my_text);
            text_true = attr.getString(R.styleable.ShadowLayoutMY_my_text_true);

            isClickable = attr.getBoolean(R.styleable.ShadowLayoutMY_my_clickable, true);
...
//            rect_bottom = shadowHeight - maxBottom;
    ...
    public void setmBackGround(Drawable drawable, String currentTag) {
        ...
        if (firstView != null && drawable != null) {
            if (mCornerRadius_leftTop == -1 && mCornerRadius_leftBottom == -1 && mCornerRadius_rightTop == -1 && mCornerRadius_rightBottom == -1) {
                PicassoUtils.setRoundCorner(firstView, drawable, mCornerRadius, currentTag);
            } else {
                int leftTop;
                if (mCornerRadius_leftTop == -1) {
                    leftTop = (int) mCornerRadius;
                ...
                PicassoUtils.setCorners(firstView, drawable, leftTop, leftBottom, rightTop, rightBottom, currentTag);
            }
        }
    }
}
