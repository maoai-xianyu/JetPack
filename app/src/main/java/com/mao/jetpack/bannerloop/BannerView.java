...
public class BannerView extends FrameLayout {
    ...
    // 8.自定义属性 - 是否使用猫眼指示器
    private boolean useMaoyanIndicator;

    // 猫眼的指示器
    private static final int INDICATOR_LIMIT = 5;
    private static final int INDICATOR_MIN_WIDTH = 40;
    private static final int INDICATOR_MAX_WIDTH = 100;
    private static final int INDICATOR_HEIGHT = 3;
    private static final int PAGER_INDICATOR_DEFAULT_BOTTOM = 7; // PagerIndicator 默认距离底部距离
    private int pagerIndicatorMarginBottom = PAGER_INDICATOR_DEFAULT_BOTTOM;
    ...
    private void initAttribute(AttributeSet attrs) {
        ...
        // 是否使用猫眼的指示器
        useMaoyanIndicator = array.getBoolean(R.styleable.BannerView_use_maoyan_indicator, false);
        array.recycle();
    }
    ...
    public void setAdapter(BannerAdapter adapter) {
        ...
        // 5.初始化点的指示器, 需要数据大于1 才可以
        if (adapter.getCount() > 1) {
            if (useMaoyanIndicator) {
                showIndicator();
            } else {
                initDotIndicator();
                // 6.Bug修复
    ...
    private void showIndicator() {
        MaoyanAdxViewPagerIndicator
            viewPagerIndicator = createIndicator(getContext(), adjustIndicatorWidth(mAdapter.getCount()), INDICATOR_HEIGHT);
     ...
     * 创建指示器
     */
    private MaoyanAdxViewPagerIndicator createIndicator(Context context, int width, int height) {
        MaoyanAdxViewPagerIndicator indicator = new MaoyanAdxViewPagerIndicator(context);
        LayoutParams layoutParams = new LayoutParams(
            (int) (getResources().getDisplayMetrics().density * width),
            (int) (getResources().getDisplayMetrics().density * height));
        layoutParams.gravity = Gravity.CENTER_HORIZONTAL | Gravity.BOTTOM;

        if (pagerIndicatorMarginBottom <= 0) {
            layoutParams.setMargins(0, 0, 0,
                (int) (getResources().getDisplayMetrics().density * PAGER_INDICATOR_DEFAULT_BOTTOM));
        } else {
            layoutParams.setMargins(0, 0, 0,
                (int) (getResources().getDisplayMetrics().density * pagerIndicatorMarginBottom));
        }
        indicator.setLayoutParams(layoutParams);
        indicator.setViewPager(mBannerVp);
        return indicator;
    }
     ...
     * 最大宽度
     */
    private int adjustIndicatorWidth(int size) {
        if (size < INDICATOR_LIMIT) {
            return INDICATOR_MIN_WIDTH;
        } else {
            return INDICATOR_MAX_WIDTH;
        }
    }
